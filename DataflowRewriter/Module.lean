...
structure Module (S : Type u₁) : Type u₁ where
  inputs : List (S → Nat → S → Prop)
  outputs : List (S → Nat → S → Prop)
...
@[simp]
def List.remove? {α : Type u} : (as : List α) → Nat → List α
  | [], _ => []
  | List.cons _ as, 0 => as
  | List.cons a as, n + 1 => a::remove? as n
...
def connect (mod : Module S) (i : Fin (List.length mod.inputs)) (o : Fin (List.length mod.outputs)) : Module S :=
       { inputs := List.remove mod.inputs i,
         ...
         internals :=  (λ st st' => 
                            ∃ consumed_output output, mod.outputs.get o st output consumed_output
                              ∧ mod.inputs.get i consumed_output output st') 
                       :: mod.internals }

@[simp]
def liftL (x: S -> T -> S -> Prop) : (S × S' -> T -> S × S' -> Prop) :=
  λ (a,b) ret (a',b') => x a ret a' /\ b = b'

@[simp]
def liftR (x: S' -> T -> S' -> Prop) : S × S' -> T -> S × S' -> Prop :=
  λ (a,b) ret (a',b') => x b ret b' /\ a = a'
...
@[simp]
def merge : Module (List Nat):=
  { inputs := [λ oldList newElement newList => newList = newElement :: oldList,
               λ oldList newElement newList => newList = newElement :: oldList],
    outputs := [ λ oldList oldElement newList => oldList = newList ++ [oldElement]],
    internals := []
  }

def composed_threemerge :=
  let merge1 := merge;
  let merge2 := merge;
...
def threemerge : Module (List Nat):=
  { inputs := [ λ oldList newElement newList => newList = newElement :: oldList,
                λ oldList newElement newList => newList = newElement :: oldList,
                λ oldList newElement newList => newList = newElement :: oldList],
    outputs := [λ oldList oldElement newList => oldList = newList ++ [oldElement]],
...
@[simp]
def getIO.{u₁} {S : Type u₁} (l: List (S -> Nat -> S -> Prop)) (n : Nat): (S -> Nat -> S -> Prop) :=
  List.getD l n (fun _ _ _ => True)
...
structure indistinguishable (init_i : I) (init_s : S) : Prop where
  ...
  new_i v,
    imod.inputs.get ident init_i v new_i →
    ∃ new_s, getIO smod.inputs ident init_s v new_s
  outputs_indistinguishable : ∀ ident new_i v,
    imod.outputs.get ident init_i v new_i →
    ∃ new_s, getIO smod.outputs ident init_s v new_s

structure comp_refines (R : I → S → Prop) (init_i : I) (init_s : S) : Prop where
  inputs :
    ∀ ident mid_i v,
      imod.inputs.get ident init_i v mid_i →
      ∃ almost_mid_s mid_s,
        getIO smod.inputs ident init_s v almost_mid_s
        ∧ existSR smod almost_mid_s mid_s
        ∧ R mid_i mid_s
        ∧ indistinguishable imod smod mid_i mid_s
  outputs :
    ∀ ident mid_i v,
      imod.outputs.get ident init_i v mid_i →
      ∃ almost_mid_s mid_s,
        getIO smod.outputs ident init_s v almost_mid_s
        ∧ existSR smod almost_mid_s mid_s
        ∧ R mid_i mid_s
        ∧ indistinguishable imod smod mid_i mid_s
  internals :
...
@[simp]
def merge_sem :=
  match build_module' mergeLow [⟨List Nat, merge⟩] with
  | some x => x
  | none => ⟨Unit, empty⟩
#reduce merge_sem
-- #eval (merge_sem Nat).2

example : ∀ (l:List Nat) j, l = [] → List.get l j = 1 → False := by
  intros l j H1 H2
  rw [H1] at j
  simp at j
  cases j
...
theorem correct_threeway {T: Type _} :
    refines (merge_sem.snd) threemerge
          (fun x y => by
              simp at x;
              exact (x.1 ++ x.2 = y)
              ) := by
