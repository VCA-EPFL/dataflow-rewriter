...
instance {T} : MatchInterface (merge_sem' T).snd (threemerge' T) where
  input_types := by apply inputs_match; solve_by_elim only [List.Perm.swap, List.Perm.trans]
  output_types := by apply outputs_match; rfl
...
theorem φ_indistinguishable {T x y} :
  φ x y → indistinguishable (merge_sem' T).snd (threemerge' T) x y := by
  unfold φ; intro H
    ...
    (merge_sem' T).snd ⊑_{φ} threemerge' T := by
      ...
      . intro ident ⟨x'1, x'2⟩ v Hcontains Himod
        ...
        · dsimp at *
          ...
          have Hindis := φ_indistinguishable Hφ
      ...
      · intro ident mid_i v Hcontains Hi
        ...
        have Hindis := φ_indistinguishable HerasePerm
        constructor; constructor; and_intros
