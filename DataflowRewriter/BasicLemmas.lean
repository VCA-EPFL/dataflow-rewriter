...
theorem rule_contains {S} {a : PortMap Ident (Σ T, S → T → S → Prop)} {ident init_i v new_i}:
  ...
  intro H
  cases h : (Batteries.AssocList.find? ident a)
  · have : ((Batteries.AssocList.find? ident a).getD ⟨PUnit.{u_3 + 1}, fun x x x => False⟩) = ⟨PUnit.{u_3 + 1}, fun x x x => False⟩ := by
      rw [h]; rfl
    rw [rw_rule_execution this] at H; simp at H
  · rw [← Batteries.AssocList.contains_find?_iff]
...
theorem EqExt_getIO {S} {m m' : PortMap Ident ((T : Type) × (S → T → S → Prop))} :
  ...
  unfold getIO Batteries.AssocList.EqExt at *; intro hext i; rw [hext]
...
theorem getIO_not_contained_false {Ident} [DecidableEq Ident] {S}
  ...
  ¬ pm.contains x1 → (pm.getIO x1).snd x2 x3 x4 → False := by
    intros H1 H2
    replace H1 := Batteries.AssocList.contains_none H1
